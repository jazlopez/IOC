...
try:
    ...
    print "INFO: Requesting google plus sites from IOC rest"
    response = requests.get('http://stage.obpplatform.com/plugin/ioc/rest.php?router=google-plus.sites')
    json_response = response.json()

    if 'error' in json_response.keys():
        raise Exception('Unable to fetch google plus sites from IOC rest')
