...
def view_corporate_page(_driver, _pageId=0, _page=""):
    ...
    try:
        ...
        print "{} has time out. Process is moving forward to the next url.\nERROR: {}".format(_error.message)
...
try:
    ...
    corporate_driver = setup_driver()

    # implicit wait
    corporate_driver = setup_implicit_wait(corporate_driver)

    request = requests.get('http://stage.obpplatform.com/plugin/ioc/rest.php?router=corporate.sites')
    ...
    for url in urls:
        ...
        view_corporate_page(corporate_driver, url['id'], url['url'])
